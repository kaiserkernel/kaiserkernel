# Hi there 👋 I'm Baftijar Gashi

### Senior Full-Stack Python Engineer • Healthcare AI • FHIR/HL7 • Software Architect

I'm a software engineer with 10+ years of experience building scalable software, AI-powered healthcare platforms, and enterprise applications.

My primary focus is designing systems that solve real business problems—from hospital interoperability platforms and AI-assisted clinical workflows to SaaS products, developer platforms, and cloud-native microservices.

Besides engineering, I also run a software company where we build custom software, AI solutions, and modern web/mobile applications for clients worldwide.

---

## 🚀 What I Do

🏥 Healthcare AI & Interoperability

- FHIR R4
- HL7 v2/v3
- SMART on FHIR
- Clinical AI
- Healthcare integrations
- HIPAA / GDPR compliant systems

🤖 Artificial Intelligence

- LLM Applications
- Retrieval-Augmented Generation (RAG)
- AI-powered workflows
- Vector Search
- Embeddings
- Clinical summarization
- AI document processing

⚙️ Backend Engineering

- Python
- FastAPI
- Django
- Flask
- Node.js
- Java Spring Boot

🌐 Frontend

- React
- Next.js
- TypeScript
- Vue.js
- Flutter

☁️ Cloud & DevOps

- Docker
- Kubernetes
- AWS
- GCP
- GitHub Actions
- Linux

🏗️ Architecture

- Microservices
- Event-driven Systems
- Domain Driven Design (DDD)
- Distributed Systems
- Multi-tenant SaaS
- High Performance APIs

---

# 💼 Current Focus

Currently building:

- AI-powered Healthcare Platforms
- FHIR/HL7 Integration Systems
- Enterprise SaaS Products
- LLM Applications
- FastAPI Backend Architectures
- Cloud Native Infrastructure
- Production AI Workflows

---

# 🛠 Tech Stack

### Languages

![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![Java](https://img.shields.io/badge/Java-E76F00?style=for-the-badge)
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=for-the-badge&logo=typescript&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-black?style=for-the-badge&logo=javascript)

### Backend

![FastAPI](https://img.shields.io/badge/FastAPI-009688?style=for-the-badge&logo=fastapi)
![Django](https://img.shields.io/badge/Django-092E20?style=for-the-badge&logo=django)
![Flask](https://img.shields.io/badge/Flask-000000?style=for-the-badge&logo=flask)
![Spring](https://img.shields.io/badge/Spring-6DB33F?style=for-the-badge&logo=spring)

### Frontend

![React](https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react)
![NextJS](https://img.shields.io/badge/Next.js-000000?style=for-the-badge&logo=nextdotjs)
![Vue](https://img.shields.io/badge/Vue-42B883?style=for-the-badge&logo=vuedotjs)
![Flutter](https://img.shields.io/badge/Flutter-02569B?style=for-the-badge&logo=flutter)

### Database

![PostgreSQL](https://img.shields.io/badge/PostgreSQL-316192?style=for-the-badge&logo=postgresql)
![MongoDB](https://img.shields.io/badge/MongoDB-47A248?style=for-the-badge&logo=mongodb)
![Redis](https://img.shields.io/badge/Redis-DC382D?style=for-the-badge&logo=redis)

### DevOps

![Docker](https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker)
![Kubernetes](https://img.shields.io/badge/Kubernetes-326CE5?style=for-the-badge&logo=kubernetes)
![AWS](https://img.shields.io/badge/AWS-232F3E?style=for-the-badge&logo=amazonaws)
![GCP](https://img.shields.io/badge/GCP-4285F4?style=for-the-badge&logo=googlecloud)

---

# 🏆 Selected Experience

## Senior Full-Stack Python Engineer

Building AI-assisted healthcare platforms that process millions of healthcare events while integrating legacy hospital systems with modern FHIR APIs.

Highlights:

- Enterprise Healthcare Systems
- AI Clinical Workflows
- HL7 → FHIR Pipelines
- Large-scale ETL
- Multi-tenant SaaS
- Distributed Microservices
- Hospital Integrations
- AI-assisted Decision Support

---

# 📈 GitHub Statistics

<p align="center">

<img height="170" src="https://github-readme-stats.vercel.app/api?username=kaiserkernel&show_icons=true&theme=tokyonight"/>

<img height="170" src="https://github-readme-stats.vercel.app/api/top-langs/?username=kaiserkernel&layout=compact&theme=tokyonight"/>

</p>

<p align="center">

<img src="https://github-readme-streak-stats.herokuapp.com/?user=kaiserkernel&theme=tokyonight"/>

</p>

---

# 🌱 Currently Learning

- Advanced AI Agents
- MCP
- Healthcare GenAI
- Kubernetes at Scale
- High-performance Python
- Distributed AI Systems

---

# 🤝 Software Company

I also run a software company focused on delivering:

- Enterprise Software
- AI Solutions
- Healthcare Platforms
- SaaS Products
- Cloud Architecture
- Mobile Applications
- Web Applications
- Backend Systems

We work with startups, healthcare companies, and enterprises around the world.

---

# 🚀 We're Hiring Remote Assistants

We're always interested in meeting talented people.

We're currently looking for remote assistants who are:

✅ Reliable

✅ Great communicators

✅ Organized

✅ Quick learners

Technical experience is a huge advantage.

Experience with software development, AI tools, programming, QA, project coordination, or technical support is especially welcome.

If you're passionate about technology and eager to grow, feel free to get in touch.

---

# 📫 Contact

📧 Email: **baftijargashi827@gmail.com**

💼 LinkedIn

https://www.linkedin.com/in/baftijar-gashi-a9101041a/

💻 GitHub

https://github.com/kaiserkernel

---

> *"Great software isn't just written—it's designed, engineered, tested, and continuously improved."*

⭐ Thanks for visiting my profile!